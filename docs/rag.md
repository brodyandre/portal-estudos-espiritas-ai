...
Chamadas simultaneas para o mesmo `manifestFingerprint` compartilham a construcao em andamento. Esses consumidores reutilizam a mesma promise e nao criam nova tentativa fisica. A promise e removida ao final da tentativa, inclusive em falha, permitindo nova tentativa posterior. O cache e local ao processo da API e nao coordena multiplas instancias.

Na Entrega 8B.1, o `governedCorpusService` tambem mantem um estado operacional em memoria, atualizado pela tentativa fisica operacional mais recentemente iniciada. Cada tentativa fisica nova recebe um `attemptId`; consumidores da mesma promise compartilham esse `attemptId`. Tentativas antigas podem continuar executando para seus chamadores, mas quando terminam fora de ordem nao publicam cache nem sobrescrevem o diagnostico mais novo. O estado inicial e `not_built`; sucessos publicados resultam em `ready` quando ha documentos e `empty` quando o manifesto valido nao possui fontes elegiveis; falhas deterministicas conhecidas, incluindo frontmatter ou parsing deterministico invalido, ficam em `invalid`; falhas de catalogo, leitura, filesystem ou dependencia ficam em `unavailable`.

Esse diagnostico separa dados publicados de tentativa operacional. `manifestSourceCount`, `documentCount`, `chunkCount`, `manifestFingerprint`, `corpusFingerprint` e `lastSuccessfulBuildAt` pertencem sempre ao ultimo snapshot publicado com sucesso. `state`, `rebuilding`, `stale`, `lastAttemptAt` e `lastFailure` descrevem a tentativa operacional mais recente. Uma tentativa falha nao publica identidade, contagens nem fingerprints parciais; antes do primeiro snapshot esses campos publicados permanecem `0` ou `null`.

As flags `rebuilding` e `stale` seguem essa separacao. `rebuilding` acompanha somente a tentativa fisica operacional mais recente: fica verdadeiro enquanto ela esta em andamento e volta para falso quando ela conclui, mesmo que uma tentativa antiga e obsoleta ainda esteja pendente. `stale` so fica verdadeiro quando ja havia um snapshot publicado e uma tentativa posterior observou falha ou mudanca sem conseguir publicar substituto valido. Nesse caso, o diagnostico ainda mostra o ultimo snapshot valido nos campos publicados, mas o snapshot anterior nao e servido pelos fluxos publicos como fallback da tentativa falha. Nao ha inspecao automatica para descobrir stale.

No startup da API, apos o servidor HTTP comecar a escutar, um bootstrap operacional assincrono chama `governedCorpusService.getSnapshot()` uma unica vez por processo. Esse bootstrap reaproveita a mesma montagem governada usada pelos fluxos publicos, nao cria `AuditLog`, nao bloqueia `/health`, nao executa retry em loop e publica snapshot somente depois de validacao completa. Durante a tentativa, `/ready` continua usando apenas o estado operacional em memoria: `not_built` e `building` ficam degradados com HTTP 200; `ready` e `empty` ficam prontos; `invalid`, `unavailable` e `stale` ficam unready.

O bootstrap registra apenas logs operacionais agregados, como inicio, sucesso, corpus vazio ou falha, com contagens e estado sanitizados. Ele nao registra conteudo Markdown, caminhos sensiveis, fingerprints completos, stack trace bruto, tokens, secrets ou `DATABASE_URL`.

O endpoint administrativo `GET /api/admin/knowledge/corpus/status` le apenas esse estado em memoria. Ele nao consulta catalogo, nao le arquivos, nao calcula fingerprints, nao cria promise e nao altera timestamps. O estado e local ao processo, e perdido em restart; apos novo startup, o bootstrap tenta reconstruir o snapshot a partir do catalogo persistido e dos arquivos governados.

O endpoint administrativo `POST /api/admin/knowledge/corpus/rebuild` executa uma reconstrucao fisica explicita, sincrona e governada. Ele exige admin, aceita somente body ausente ou `{}`, rejeita qualquer query parameter e nao permite escolher caminhos, arquivos, fontes, provider, modo async ou bypass de governanca. A operacao reler manifesto e documentos aprovados, recalcula as identidades editorial e fisica, valida frontmatter/conteudo/hashes e publica um novo snapshot somente apos sucesso completo. Em falha, nenhum snapshot parcial e publicado e o estado operacional fica `invalid` ou `unavailable`, com `stale=true` quando ja havia snapshot anterior.

O rebuild administrativo ignora cache hit: mesmo que `manifestFingerprint` e `corpusFingerprint` terminem identicos, a leitura fisica aconteceu, um novo snapshot em memoria e publicado, `lastSuccessfulBuildAt` e atualizado e `lastFailure` e limpo. O fingerprint nao e alterado artificialmente para provar rebuild.

Somente um rebuild administrativo pode rodar por processo. O lock e em memoria, adquirido antes da leitura fisica e liberado em `finally`; nao ha fila, espera, coalescencia silenciosa, job persistido nem lock distribuido. Uma segunda chamada administrativa simultanea retorna `409 KNOWLEDGE_CORPUS_REBUILD_IN_PROGRESS`. Chamadas publicas novas durante um rebuild observam a promise em andamento para o fingerprint correspondente; tentativas antigas podem concluir para seus chamadores originais, mas os guardas por `attemptId` impedem publicacao regressiva de cache ou diagnostico.

O retriever continua lazy. O `POST` publica somente o snapshot; o `governedRetrieverService` reconstroi o indice no proximo acesso quando a chave composta muda. Se a chave composta permanecer igual, a instancia antiga do retriever pode ser reutilizada porque o conteudo fisico validado e identico.

Cada rebuild autenticado valido consome rate limit dedicado de 3 tentativas por administrador a cada 15 minutos, com `Retry-After`. A operacao registra auditoria persistente em `AuditLog` com eventos `KNOWLEDGE_CORPUS_REBUILD_REQUESTED`, `KNOWLEDGE_CORPUS_REBUILD_SUCCEEDED` e `KNOWLEDGE_CORPUS_REBUILD_FAILED`, usando metadados agregados e sanitizados. A auditoria nao armazena conteudo, paths, frontmatter, hashes individuais, fingerprints completos, stack trace, mensagens cruas de erro, tokens ou credenciais. Falha da auditoria inicial impede a reconstrucao; falha da auditoria terminal retorna erro administrativo sem desfazer o resultado fisico ja consolidado.

Nao ha watcher de filesystem, polling, cache distribuido, coordenacao entre instancias, Redis, worker, fila, job persistido, embeddings, vector database nem chamada a LLM na reconstrucao. Em cold start ou restart, estado, lock e rate limit locais sao perdidos; a auditoria persistente permanece no banco, mas nao transforma o rebuild em job retomavel. O bootstrap automatico apenas tenta recompor o snapshot em memoria do processo novo.
