...
O endpoint administrativo `GET /api/admin/knowledge/corpus/status` le apenas esse estado em memoria. Ele nao consulta catalogo, nao le arquivos, nao calcula fingerprints, nao cria promise e nao altera timestamps. O estado e local ao processo, e perdido em restart.
...
Nao ha watcher de filesystem, polling, cache distribuido, coordenacao entre instancias, Redis, worker, fila, job persistido, embeddings, vector database nem chamada a LLM na reconstrucao. Em cold start ou restart, estado, lock e rate limit locais sao perdidos; a auditoria persistente permanece no banco, mas nao transforma o rebuild em job retomavel.
